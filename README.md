...
> **Demo video (4 min):** [demo_video.mp4 on Google Drive](https://drive.google.com/file/d/1mw5StwI4WUdNazgXEfe4CCCfvPfif6PT/view?usp=sharing).
> Starting from an empty system, it uploads both task files through Swagger, shows async
> processing (including OCR of the image-only PDF), runs semantic queries against each file,
> and deletes a document. Step-by-step screenshots are in [docs/screenshots.md](docs/screenshots.md).
...
| Proof of execution on the two task files | [Demo video](https://drive.google.com/file/d/1mw5StwI4WUdNazgXEfe4CCCfvPfif6PT/view?usp=sharing) · [screenshots](docs/screenshots.md) · [full query transcript](docs/demo_output.md) (generated by `scripts/demo.py` through the HTTP API) |
